...
describe('E2E: specify init command validation', () => {
  ...
  describe('Argument validation (no network needed)', () => {
    it('fails with invalid AI assistant name', () => {
      expect(() => {
        execSync(
          `node "${CLI_PATH}" init "${projectDir}" --ai invalid-ai --no-git`,
          {
            encoding: 'utf-8',
            cwd: PROJECT_ROOT,
            timeout: 30000,
    ...
    it('warns when directory is not empty without --force', () => {
      ...
      expect(() => {
        execSync(
          `node "${CLI_PATH}" init "${projectDir}" --ai copilot --no-git`,
          {
            encoding: 'utf-8',
            cwd: PROJECT_ROOT,
            timeout: 30000,
  ...
  describe('Configuration display (no network needed with js)', () => {
    it('shows project configuration in output', async () => {
      ...
      try {
        result = execSync(
          `node "${CLI_PATH}" init "${projectDir}" --ai copilot --no-git --force`,
          {
            encoding: 'utf-8',
            cwd: PROJECT_ROOT,
            timeout: 120000,
  ...
  describe.skipIf(!isNetworkAvailable() || !hasGitHubToken)('Full workflow: init → create-feature → setup-plan → check → update-context', () => {
    ...
    it('completes full SDD workflow with GitHub Copilot and js scripts', async () => {
      ...
      const initResult = execSync(
        `node "${CLI_PATH}" init "${projectDir}" --ai copilot --force`,
        {
          encoding: 'utf-8',
          cwd: PROJECT_ROOT,
          timeout: 120000,
