...
describe('Complete Flow (Unix only)', () => {
  ...
  it('skips files without shebang', () => {
    if (isWindows()) return;
    ...
    try {
      ...
      const scriptPath = join(scriptsDir, 'no-shebang.sh');
      writeFileSync(scriptPath, 'echo hello'); // No shebang
      chmodSync(scriptPath, 0o644);

      const tracker = new StepTracker('Test');
      ensureExecutableScripts(tempDir, tracker);

      // Should still not be executable
      expect(isExecutable(scriptPath)).toBe(false);
    } finally {
      cleanupTempDir(tempDir);
    }
  ...
  it('skips already executable files', () => {
    if (isWindows()) return;
    ...
    try {
      ...
      const scriptPath = join(scriptsDir, 'already-exec.sh');
      writeFileSync(scriptPath, '#!/bin/bash\necho hello');
      chmodSync(scriptPath, 0o755); // Already executable

      const originalStat = statSync(scriptPath);

      const tracker = new StepTracker('Test');
      ensureExecutableScripts(tempDir, tracker);

      // Mode should be unchanged
      const newStat = statSync(scriptPath);
...
describe('isExecutable', () => {
  ...
  it('returns false for non-executable file', () => {
    ...
    const tempDir = createTempDir();
    try {
      const filePath = join(tempDir, 'not-exec.sh');
      writeFileSync(filePath, '#!/bin/bash');
  ...
  it('detects any execute bit (owner, group, or others)', () => {
    if (isWindows()) return;

    const tempDir = createTempDir();
    try {
      const filePath = join(tempDir, 'group-exec.sh');
      writeFileSync(filePath, '#!/bin/bash');
      
      // Only group execute bit
      chmodSync(filePath, 0o614); // rw---x-r--
      expect(isExecutable(filePath)).toBe(true);
    } finally {
      cleanupTempDir(tempDir);
    }
...
describe('nested scripts', () => {
  it('finds scripts in deeply nested directories', () => {
    const tempDir = createTempDir();
    try {
      const deepDir = join(tempDir, 'a', 'b', 'c', 'd');
      mkdirSync(deepDir, { recursive: true });
      writeFileSync(join(deepDir, 'deep.sh'), '#!/bin/bash');

      const scripts = findShellScripts(tempDir);
      expect(scripts).toHaveLength(1);
      expect(scripts[0]).toContain('deep.sh');
    } finally {
      cleanupTempDir(tempDir);
    }
  });

  it('finds multiple scripts at various levels', () => {
    const tempDir = createTempDir();
    try {
      writeFileSync(join(tempDir, 'root.sh'), '#!/bin/bash');
      mkdirSync(join(tempDir, 'level1'));
      writeFileSync(join(tempDir, 'level1', 'l1.sh'), '#!/bin/bash');
      mkdirSync(join(tempDir, 'level1', 'level2'));
      writeFileSync(join(tempDir, 'level1', 'level2', 'l2.sh'), '#!/bin/bash');
